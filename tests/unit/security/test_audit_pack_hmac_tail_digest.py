...
guard -- narrowing it from ``and`` to ``or``, say -- would either raise
(``entry["hmac"]`` on a non-subscriptable-by-string value) or mis-quote the
evidence pack, and nothing here would have noticed.
"""
...
def test_non_dict_and_malformed_lines_do_not_break_tail_selection(tmp_path: Path) -> None:
    """The load-bearing case: a mix of JSON types and one malformed line,
    with a valid dict-with-hmac entry on either side.

    Every JSON type ``isinstance(entry, dict)`` exists to exclude is present
    here, plus a line that fails to parse at all. The guard must skip all of
    them without raising, the malformed line must not stop the scan, and the
