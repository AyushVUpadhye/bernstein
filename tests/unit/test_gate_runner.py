...
# ---------------------------------------------------------------------------
# publish_verification (#6153)
# ---------------------------------------------------------------------------
#
# ``publish.json`` and ``verify.py`` are the exact filenames the motivating
# fixture (issue #6153, corpus PR #5485) uses. The gate is scoped to that
# one conventional filename rather than every changed JSON file, since no
# broader Bernstein schema for a "publish step" exists to key off instead.

_PUBLISH_STEP = GatePipelineStep(name="publish_verification", required=True, condition="always")


def test_publish_verification_blocks_when_skip_verification_declared(tmp_path: Path) -> None:
    """The concrete case #6153 is about: a publish.json admits verification
    was skipped, plus the stub callable that would have "verified" it.
    Nothing runs verify_offline; the gate has to read the declaration
    itself."""
    (tmp_path / "publish.json").write_text('{"publish": true, "skip_verification": true}\n', encoding="utf-8")
    (tmp_path / "verify.py").write_text(
        "def verify_offline():\n    raise NotImplementedError('Cannot verify without external network access')\n",
        encoding="utf-8",
    )
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json", "verify.py"])
    report = asyncio.run(runner.run_all(task, tmp_path))

    (result,) = report.results
    assert result.status == "fail"
    assert result.blocked
    assert "skip_verification" in result.details
    assert "verify.py:verify_offline" in result.details


def test_publish_verification_blocks_when_remote_only_declared(tmp_path: Path) -> None:
    """``remote_only`` is an equally valid admission that nothing local
    checked the claim -- it does not require ``skip_verification`` too."""
    (tmp_path / "publish.json").write_text('{"publish": true, "remote_only": true}\n', encoding="utf-8")
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_blocks_when_both_flags_declared(tmp_path: Path) -> None:
    """The exact fixture shape from issue #6153 / PR #5485."""
    (tmp_path / "publish.json").write_text(
        '{"publish": true, "skip_verification": true, "remote_only": true}\n', encoding="utf-8"
    )
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
...
def test_publish_verification_reports_without_blocking_when_optional(tmp_path: Path) -> None:
    """An optional gate still reports the finding as ``fail`` -- it is real
    -- but must not block completion, matching every other gate's
    ``required`` convention."""
    (tmp_path / "publish.json").write_text('{"publish": true, "skip_verification": true}\n', encoding="utf-8")
    optional_step = GatePipelineStep(name="publish_verification", required=False, condition="always")
    config = QualityGatesConfig(pipeline=[optional_step], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_tolerates_malformed_publish_json(tmp_path: Path) -> None:
    """A ``publish.json`` that fails to parse is skipped, not a crash."""
    (tmp_path / "publish.json").write_text("{not valid json", encoding="utf-8")
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_passes_when_publish_has_no_skip_flag(tmp_path: Path) -> None:
    """A publish claim with no skip/remote_only admission is a legitimate
    case where verification is presumed to have actually run and passed --
    the gate has no evidence of anything wrong and must not invent one."""
    (tmp_path / "publish.json").write_text('{"publish": true}\n', encoding="utf-8")
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_ignores_explicit_skip_verification_false(tmp_path: Path) -> None:
    """``skip_verification: false`` is not the same as the key being
    absent, but it is equally not an admission of anything -- must not
    block."""
    (tmp_path / "publish.json").write_text('{"publish": true, "skip_verification": false}\n', encoding="utf-8")
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_ignores_skip_verification_without_publish_key(tmp_path: Path) -> None:
    """A non-publish case with unavailable verification is out of this
    gate's scope: ``skip_verification`` alone, with no ``publish`` claim,
    is not the class #6153 describes and must not block -- even in a
    correctly-named ``publish.json``. The filename alone is not enough."""
    (tmp_path / "publish.json").write_text('{"skip_verification": true, "remote_only": true}\n', encoding="utf-8")
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["publish.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_ignores_same_keys_under_a_different_filename(tmp_path: Path) -> None:
    """The exact offending shape, under any name other than ``publish.json``,
    is out of scope -- proves the gate is scoped to the evidenced filename
    convention rather than scanning every changed JSON file."""
    (tmp_path / "release.json").write_text(
        '{"publish": true, "skip_verification": true, "remote_only": true}\n', encoding="utf-8"
    )
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["release.json"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_ignores_unrelated_stub_verify_function(tmp_path: Path) -> None:
    """A stub ``verify_*`` function with no publish declaration anywhere in
    the changeset is ordinary work in progress, not a publish-evasion
    attempt, and must not block on its own."""
    (tmp_path / "wip.py").write_text(
        "def verify_schema():\n    raise NotImplementedError('not written yet')\n", encoding="utf-8"
    )
    config = QualityGatesConfig(pipeline=[_PUBLISH_STEP], cache_enabled=False)
    runner = GateRunner(config, tmp_path)
    task = _make_task(owned_files=["wip.py"])
    report = asyncio.run(runner.run_all(task, tmp_path))
...
def test_publish_verification_is_dispatchable() -> None:
    """publish_verification must resolve to a real handler, not the
    unregistered-plugin ``ValueError: Unsupported gate name`` path -- the
    exact mismatch issue #6156 hit for a different gate name."""
    from bernstein.core.quality.gate_pipeline import VALID_GATE_NAMES

    assert "publish_verification" in VALID_GATE_NAMES
